...
def log_likelihood(fl, afb, _bin):
    ...
    normalised_scalar_array = d2gamma_p_d2q2_dcostheta(
        fl=fl, afb=afb, cos_theta_l=ctl)
    with np.errstate(invalid='ignore'): # ignore 'invalid log' message
        NLL = -np.log(normalised_scalar_array).sum()
    return NLL
...
ax[0].errorbar(range(10), fits['fls'], yerr=fits['fl_errs'], fmt='k.', ms=5)
ax[0].errorbar(range(10), FL_aval, yerr=FL_aerr, fmt='r.', ms=5)
ax[0].set(xlabel='Bin number', ylabel=r'$F_L$', xticks=range(10))
ax[0].grid()
# ax[0].plt.xticks(np.arange(min(x), max(x)+1, 1.0))

ax[1].errorbar(range(10), fits['afbs'], yerr=fits['afb_errs'], fmt='k.', ms=5)
ax[1].errorbar(range(10), AFB_aval, yerr=AFB_aerr, fmt='r.', ms=5)
ax[1].grid()
ax[1].set(xlabel='Bin number', ylabel=r'$A_{FB}$', xticks=range(10))

fig.suptitle('Our fit (black), "actual" data (red)', size=17)
