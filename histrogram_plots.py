...
def plot_hist_quantity(df,column,bins=100,range=None,label=None,xlabel=None):
...
def plot_hist_quantity_new(df,column,bins=100,range=None,label=None,xlabel=None):
    # Prevents exception being thrown for 'year' column
    if df[column].dtype != 'object':
...
def generic_selector_plot_new(orginal,subset, not_subset, column, bins = 100, show = True,columnname=None):
    bins, h = plot_hist_quantity(orginal, column, label='Initial', bins=bins,xlabel=columnname)
    plot_hist_quantity(subset, column, label='Accepted', bins = bins,xlabel=columnname)
    plot_hist_quantity(not_subset, column, label='Rejected', bins=bins,xlabel=columnname)
    plt.legend()
    if show:
        plt.show()
